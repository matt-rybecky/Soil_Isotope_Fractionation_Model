...
class ParameterRangeWidget:
    """Reusable widget for parameter range input with validation."""
    
    def __init__(self, parent: tk.Widget, parameter_name: str, 
                 default_min: float, default_max: float, default_guess: float,
                 row: int = None):
        """
        Create parameter range input widget.
        ...
        Args:
            ...
            default_guess: Default initial guess
            row: Grid row for layout
        """
        self.parent = parent
        ...
        self.guess_var = tk.DoubleVar(value=default_guess)
        self.enabled_var = tk.BooleanVar(value=False)
        
        # Create widgets
        self._create_widgets(row)
    
    def _create_widgets(self, row: int):
        """Create the parameter range widgets."""
        if row is not None:
            # Checkbox
            self.checkbox = tk.Checkbutton(self.parent, text=self.parameter_name,
                                         variable=self.enabled_var,
                                         font=StandardStyles.BODY_FONT)
            self.checkbox.grid(row=row, column=0, sticky="w", padx=5)
            
            # Min entry
            self.min_entry = tk.Entry(self.parent, textvariable=self.min_var, width=8)
            self.min_entry.grid(row=row, column=1, padx=2)
            
            # Max entry
            self.max_entry = tk.Entry(self.parent, textvariable=self.max_var, width=8)
            self.max_entry.grid(row=row, column=2, padx=2)
            
            # Guess entry
            self.guess_entry = tk.Entry(self.parent, textvariable=self.guess_var, width=8)
            self.guess_entry.grid(row=row, column=3, padx=2)
    
    def get_values(self) -> Dict[str, Any]:
        """Get current parameter range values."""
        return {
    ...
    def set_values(self, enabled: bool, min_val: float, max_val: float, guess: float):
        """Set parameter range values."""
        self.enabled_var.set(enabled)
        self.min_var.set(min_val)
        self.max_var.set(max_val)
        self.guess_var.set(guess)
    ...
    def validate(self) -> Tuple[bool, str]:
        ...
        guess_val = self.guess_var.get()
        
        if min_val >= max_val:
            return False, f"{self.parameter_name}: minimum must be less than maximum"
        
        if not (min_val <= guess_val <= max_val):
            return False, f"{self.parameter_name}: initial guess must be between min and max"
